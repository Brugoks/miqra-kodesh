...
**The guided walk takes the screen.** Starting the tour is the visitor saying "you drive", so `.scene-page--touring` stands the driving controls down — the vantage rail, the hours, the thumbstick and the labelled buttons — and the info panel becomes a caption on the bottom edge with a chevron that pushes it aside entirely. Closing the panel is not enough on its own, because the next stop opens it again; that is right for a tour and wrong for someone who has asked for the view, hence the separate collapsed state. All of it is one class in `Scene.css` rather than state in the render loop, and it all comes back the moment the walk stops.
