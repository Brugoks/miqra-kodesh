...
**The guided walk (`src/lib/sceneNarration.js`, `useSceneTour.js`).** Walks the vantages in order, reading each one's blurb aloud through the existing `fish-tts` edge function, which caches every line — so a scene's tour is paid for once. The silent path is not a consolation prize: with no session, no voices, or a failed synthesis, the same tour runs and dwells on each line for as long as it takes to read. Cancellation is the hard part — one `AbortController` threads through the flight, the synthesis round-trip, the audio element and every dwell, and taking the controls (walking, or picking a vantage) stops the walk.
