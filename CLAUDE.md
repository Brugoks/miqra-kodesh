...
**Shared presence layer.** Four modules do the same job for every scene, so adding immersion is one change rather than four:

- `sceneLighting.js` owns the sky shader, the sun and the hour of the day. The sun is placed by **compass bearing and elevation**, not by hand — the scenes genuinely disagree about handedness (the temple has +X north, Capernaum +X east), and a hand-placed sun is wrong in a way that only shows up as mirrored shadows. `SCENE_AXES` mirrors the manifests' `geo`, and a test asserts they still agree. `morning` is solved from the temple's original hand-placed sun so the default view of every scene is unchanged; the other four hours are additions. `setTimeOfDay` re-points everything without rebuilding, and builders read `lighting.current.lamps` in their `update()` so lamplight tracks the hour.
- `sceneFigures.js` — people. One reusable rig of Object3Ds is posed per figure per frame and read out as world matrices into four InstancedMeshes (robe, head, headcloth, arms), so any number of people costs four draw calls. `ACTIVITIES` is a table of pure `(t, phase, cadence) => pose` functions; `gather()` and `scatter()` place them, and **grouping is the point** — uniform random scatter is what made the old cone-people read as traffic cones.
- `sceneProps.js` — the jars, baskets, rope and nets that make a place look lived in. Instanced by kind. Deliberately **not** in the collision model: walking through a basket is a smaller lie than an empty scene, and a hundred extra colliders would cost every step.
- `scenePostProcessing.js` — GTAO, a tight bloom, `OutputPass`, then vignette and grain. Gated to `quality === 'high'`. The AO is the load-bearing one: without contact darkening nothing sits *on* anything. `OutputPass` is not optional decoration — once a composer is in the chain the renderer stops tone-mapping, so removing it does not soften an effect, it washes out the picture.

**Sound (`src/lib/sceneAudio.js`).** Every sound is synthesised from Web Audio primitives — filtered pink noise, oscillators, envelopes — and nothing is fetched, the same trade the geometry makes. Positional sources use a `PannerNode`; footsteps are driven by the walk cadence in `Scene.jsx` (not a timer) and pick their timbre from the walker's `region`. Nothing is on a timer at all: `update(elapsed, listener)` is called once per frame from the render loop that already exists, which keeps sparse events deterministic under test. A context must be created inside a user gesture, so the soundscape is built on the "Step inside" click and nowhere else. Tests drive the whole graph through a fake `AudioContext` — note that a fake must not use `type` as its discriminator, because `BiquadFilterNode.type` and `OscillatorNode.type` are real properties the module assigns to.

**Framing (`sceneFraming.js`).** A three.js camera's `fov` is *vertical*, but "I can't see what is going on" is a complaint about the *horizontal* field, and the two only agree at one aspect ratio. Every vantage was composed on a landscape screen, so a fixed 60° hands a portrait phone under 30° across — a keyhole, and worst exactly where the standpoint is closest (two metres from the veil, or from Levi's tax booth). Both the opening fov and the zoom-out stop are therefore derived from the viewport's aspect: widen the vertical field towards whatever would reproduce the reference screen's horizontal field, clamped at `FOV_MAX_NARROW` because the honest answer on a phone is past 120° and would bow the architecture. **The default lands on that clamp, so a phone opens fully zoomed out and the pinch can only zoom in** — which is the right way round for the device whose problem is seeing too little. Screens at or wider than the reference are untouched, so nothing about the desktop view moves. `engine.fovUser` records that the visitor has pinched or scrolled, so a rotation re-derives the default only while it is still the default — but always re-clamps, since portrait may zoom out further than landscape.

**Embodiment.** The camera is a body, not a tripod: head bob driven by *distance walked* rather than time (so the cadence is right at any speed and stops dead when you do), a spring in the knees on a step down, breathing while idle, and an additive FOV kick when running — additive because `engine.fov` is the visitor's own pinch-zoom and must survive a sprint. All of it honours `prefers-reduced-motion`; footsteps do not, because for that visitor the sound is the main thing saying they moved.

**The guided walk (`src/lib/sceneNarration.js`, `useSceneTour.js`).** Walks the vantages in order, reading each one's blurb aloud. The narration is **pre-recorded, not synthesised at runtime**: `scripts/build-scene-narration.js` reads every vantage blurb in Rico's voice through `fish-tts` and commits the mp3s under `public/assets/scenes/<slug>/narration/`, indexed by `src/lib/sceneNarrationManifest.js` (generated — don't hand-edit). So a stop plays a static file off the origin: no round-trip after the camera lands, no session required, nothing billed per visit, and the service worker keeps it. It also gets the voice we actually want — Rico is a *restricted* voice in `fish-tts`, hidden from the browser-facing picker, so a runtime request for it is silently answered with someone else.

Filenames carry a hash of the voice and the exact text, which is the whole safety property: **edit a blurb and the lookup misses rather than serving a recording of the old words.** A miss falls through to a live `fish-tts` synthesis (in whatever voice the caller is allowed), and then to the silent path, which is not a consolation prize — the same tour runs and dwells on each line for as long as it takes to read. `sceneNarrationManifest.test.js` is what notices a blurb edited without a re-record; re-record with `node scripts/build-scene-narration.js` (re-runs are free — `fish-tts` caches every line it has ever made).

Cancellation is the hard part — one `AbortController` threads through the flight, the synthesis round-trip, the audio element and every dwell, and taking the controls (walking, or picking a vantage) stops the walk.

**Reading one line aloud.** The walk is not the only way to hear the voice: every panel — vantage, hotspot or barrier — carries a speaker button, so a visitor exploring on their own can ask for the words where they are standing instead of surrendering the camera for a whole tour. `speakLine()` is that one rung-by-rung ladder (build-time recording, then a live synthesis, then silence) and `runTour` goes down the same function, so anything that works for the button works for the walk. Only a vantage has a recording — hotspots have never been through the narration build — so those land on synthesis, and on nothing where that is unavailable, which costs the button nothing and is why it is offered on all three. `useSceneTour` owns both voices with **two** AbortControllers: the walk's, and the reading's. They are separate so that changing panels can silence a line without ending a walk, and shared enough that starting either one stops the other — two mouths on the same sentence is the single thing neither control may produce.
...
Also worth keeping intact: `sceneLighting.test.js` asserts the axes match the manifests and that `morning` still reproduces the original sun — both are bugs that look fine and are wrong; and the scene chrome (`Scene.css`) is dark in both themes by design, one of the documented exceptions to the token rule.
...
**The guided walk takes the screen.** Starting the tour is the visitor saying "you drive", so `.scene-page--touring` stands the driving controls down — the vantage rail, the hours, the thumbstick and the labelled buttons — and the info panel becomes a caption with a chevron that pushes it aside entirely. Closing the panel is not enough on its own, because the next stop opens it again; that is right for a tour and wrong for someone who has asked for the view, hence the separate collapsed state. All of it is one class in `Scene.css` rather than state in the render loop, and it all comes back the moment the walk stops.

**Where the description sits.** On a phone the panel is the third row at the top, not a bottom sheet, and it opens **folded to its title line** with a "Tap to read" peek — the whole folded card is the target, since a title line is small for a thumb. Two reasons for the top: the bottom belongs to the thumb (the vantage rail, the walk button and the stick are all down there and were crowding each other), and what is worth looking at in these scenes is at eye level and below while the top of the frame is mostly sky. The collapse is sticky across vantages on purpose — "stop putting text over the scene" is a standing instruction, not a per-stop one. `isPhoneViewport()` asks the same 760px of the same media engine the stylesheet uses, so the layout and the state that assumes it cannot disagree.
...
- A build script that needs an edge function to treat it as service-role (only `fish-tts` does, for restricted voices) must present the project's **new-format** secret key — the edge runtime injects `sb_secret_…` as `SUPABASE_SERVICE_ROLE_KEY`, not the legacy `service_role` JWT. That key is `SUPABASE_SECRET_KEY` in `.env`.
