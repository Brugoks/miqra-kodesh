-- Redefine dev_usage_snapshot() to use tup_returned from pg_stat_database for egress estimation.
-- This prevents counting internal sequential scans (rows read to evaluate WHERE clauses) and matches actual network egress.
...
begin
  ...
  select coalesce(xact_commit, 0), coalesce(xact_rollback, 0), coalesce(tup_returned, 0)
  into db_commits, db_rollbacks, db_tup_returned
  from pg_stat_database
  where datname = current_database();

  -- Calculate estimated egress bytes: 1100 bytes overhead per transaction/REST query, plus 300 bytes per tuple returned to client.
  est_egress_bytes := (db_commits + db_rollbacks) * 1100 + db_tup_returned * 300;
