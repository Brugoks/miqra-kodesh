...
import {
  ...
  AlertTriangle, ChevronDown, ChevronUp, Quote, Copy, FileJson, Save,
} from 'lucide-react';
...
function ManualBereanWorkflow({ talk, report, disabled, onSaved }) {
  const [open, setOpen] = useState(false);
  ...
  async function prepareExtract() {
    ...
    if (data?.kit) {
      ...
      setStatus('Extraction prompt ready.');
    }
  }
  ...
  async function prepareJudge() {
    try {
      ...
      if (data?.kit) {
        ...
        setStatus('Scripture judgment prompt ready.');
      }
    } catch (err) {
  ...
  async function saveScripture() {
    try {
      ...
      const data = await invokeManual('manual-save-scripture', { extractionResult, judgmentResult });
      if (data?.analysis) {
        onSaved(data.analysis, 'scripture');
        setStatus('Manual Scripture review saved.');
      }
    } catch (err) {
      setError(err.message);
    }
  ...
  async function saveIllustrations() {
    try {
      ...
      const data = await invokeManual('manual-save-illustrations', { illustrationResult });
      if (data?.analysis) {
        ...
        <div className="berean-manual-body">
          <div className="berean-manual-step">
            <div className="berean-manual-step-head">
              <span>1. Scripture extraction</span>
              <button className="btn-secondary berean-manual-btn" onClick={prepareExtract} disabled={disabled || !!busy}>
                <FileJson size={13} /> {busy === 'manual-extract-kit' ? 'Preparing…' : 'Prepare prompt'}
              </button>
            </div>
            {extractKit && (
              <>
            ...
            <textarea
              ...
              value={extractJson}
              onChange={e => setExtractJson(e.target.value)}
          ...
          <div className="berean-manual-step">
            ...
            <textarea
              ...
              placeholder="Paste Scripture judgment JSON here"
              rows={6}
            />
            <button
          ...
          <div className="berean-manual-step">
            ...
            <textarea
              ...
              placeholder="Paste Examples & stories JSON here"
              rows={6}
            />
            <button
              className="btn-primary berean-manual-save"
